document.addEventListener("DOMContentLoaded", function () {
  ...
  toggleLabel.style.background = "inherit";
  ...
  const fetchWidthHeightMap = async () => {
    const response = await fetch(`./test/images/ass_layout_w_h.json`);
  ...
  const updateBackgroundImage = (widthHeightMap) => {
    ...
    if (w >= 392 && w <= 1600) {
      ...
      if (h) {
        const imagePath = `./test/images/ass_layout_${h}_${w}.webp`;
