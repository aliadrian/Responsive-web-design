...
function minifyJs(js) {
  js = normalizeLineBreaks(js);
  return js.replace(/\s+/g, ' ').replace(/\s*([{}:;,])\s*/g, "$1").trim();
}

function minifyContent(content, fileType) {
  console.log('Entered minifyContent function');
  switch (fileType) {
    case "html":
...
studentFiles.forEach((value, key) => {
  referenceFiles.set(`test/ref/${key}`, value);
});

async function fetchFileContent(file, fileType) {
  ...
  try {
    ...
    console.log(`Fetched ${file} of type ${fileType}:`, content);
    return minifyContent(content, fileType);
  } catch (error) {
...
async function loadFiles() {
  ...
  const studentPromises = Array.from(studentFiles.keys()).map((file, index) => {
    ...
    console.log(`Fetching student file ${index + 1}: ${file}, type: ${fileType}`);
    return fetchFileContent(file, fileType).then(content => {
      if(content !== null) {
  ...
  const referencePromises = Array.from(referenceFiles.keys()).map((file, index) => {
    const fileType = file.split('.').pop().split('/').pop();
    console.log(`Fetching reference file ${index + 1}: ${file}, type: ${fileType}`);
    return fetchFileContent(file, fileType).then(content => {
      if(content !== null) {
...
function readRulesFromReferenceFiles(referenceFiles) {
  ...
  const ruleFiles = [
    "test/ref/css/flexbox.css",
    "test/ref/css/css-grid.css",
    "test/ref/css/css-grid-area.css",
  ];
  let rules = {};
...
function validateStudentCss(studentFiles, rules) {
  ...
  for (const [file, css] of studentFiles) {
    ...
    let strippedFile = `test/ref/${file}`;
...
function generateFeedbackHtml(validationResults) {
  ...
  let feedbackHtml = `
                  <h2 style="font-size: 2rem; margin-bottom: 1rem;">Rules</h2>
                  <table style="width: 100%; border-collapse: collapse; margin-top: 1rem;">
                    <thead style="background-color: #f2f2f2;">
                      <tr>
                        <th style="font-size: 1rem; text-align: left; word-wrap: break-all; padding: 10px; border: 1px solid #ccc;">File</th>
                        <th style="font-size: 1rem; text-align: left; word-wrap: break-all; padding: 10px; border: 1px solid #ccc;">Prop&shy;erties</th>
                        <th style="font-size: 1rem; text-align: left; word-wrap: break-all; padding: 10px; border: 1px solid #ccc;">Declar&shy;ations</th>
                        <th style="font-size: 1rem; text-align: left; word-wrap: break-all; padding: 10px; border: 1px solid #ccc;">Media Queries</th>
                      </tr>
                    </thead>
                    <tbody>
                    `;
  ...
  for (let [file, result] of Object.entries(validationResults)) {
    let fileName = file.replace("test/ref/css/", "").replace(".css", "").replace("-", " ");
    ...
    for (let [rule, value] of Object.entries(result)) {
      if (rule === "invalidProperties") {
        if (value[0] === "Empty file") {
          propertiesStatus = '<span style="color: #F5A;">File is Empty!</span>';
        } else {
          if (value.length > 0) {
            propertiesStatus = '<span style="font-size: 1rem; color: #F5A;">Not allowed:</span>';
            propertiesStatus += '<ul style="font-size: 1rem; margin: 0; padding-left: 20px;">';
            for (let prop of value) {
              propertiesStatus += `<li style="font-size: 1rem; text-align: left; word-wrap: break-all;">${prop}</li>`;
            }
            propertiesStatus += '</ul>';
          } else {
            propertiesStatus = '<span style="color: green;">No rule broken</span>';
          }
        }
      } else if (rule === "mandatoryDeclarations") {
          if (value[0] === "Empty file") {
            declarationsStatus = '<span style="color: #F5A;">File is Empty!</span>';
          } else {
              declarationsStatus = value ? '<span style="font-size: 1rem; color: green;">No rule broken</span>' : '<span style="color: #F5A;">Failed</span>';
          }
        }
        else {
          if (value[0] === "Empty file") {
            mediaQueriesStatus = '<span style="color: #F5A;">File is Empty!</span>';
          } else if (value[0] === "No Media Query") {
            mediaQueriesStatus = '<span style="color: #F5A;">None Present</span>';
          } else if (value[0] === "Requires Two") {
            mediaQueriesStatus = '<span style="color: #F5A;">Requires Two</span>';
          } else {
            mediaQueriesStatus = value ? '<span style="font-size: 1rem; color: green;">No rule broken</span>' : '<span style="color: #F5A;">Failed</span>';
          }
      }
    }

    feedbackHtml += `<tr style="border: 1px solid #ccc;">`;
    feedbackHtml += `<td style="font-size: 1rem; text-transform: uppercase; text-align: left; word-wrap: break-all; padding: 10px; border: 1px solid #ccc;">${fileName}</td>`;
    feedbackHtml += `<td style="font-size: 1rem; text-align: left; word-wrap: break-all; padding: 10px; border: 1px solid #ccc;">${propertiesStatus}</td>`;
    feedbackHtml += `<td style="font-size: 1rem; text-align: left; word-wrap: break-all; padding: 10px; border: 1px solid #ccc;">${declarationsStatus}</td>`;
    feedbackHtml += `<td style="font-size: 1rem; text-align: left; word-wrap: break-all; padding: 10px; border: 1px solid #ccc;">${mediaQueriesStatus}</td>`;
    feedbackHtml += `</tr>`;
  }
...
function generateButtonAndModal() {
  ...
  button.style.borderBottomRightRadius = '100vmax';
  button.style.background = "#F5A";
  ...
  closeButton.style.backgroundColor = '#F5A';
  ...
  outputDiv.style.backgroundColor = '#fefefe';
